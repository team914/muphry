...
/**
 * @brief      for converting T to double of 1, 0, or -1 respectively
 *
 * @param[in]  val   The value
 *
 * @tparam     T     a number type
 *
 * @return     double that is 1, 0, -1
 */
...
Chassis::Chassis() {
  ...
  odom =
    std::make_shared<TwoEncoderOdometry>(TimeUtilFactory().create(), skidSteerModel, adiScales);
  ...
  pidController = std::make_shared<ChassisControllerPID>(
    ...
    std::make_unique<IterativePosPIDController>(
      chassisDistancekP, chassisDistancekI, chassisDistancekD, 0, chassisDistanceTimeUtil),
    std::make_unique<IterativePosPIDController>(
      chassisTurnkP, chassisTurnkI, chassisTurnkD, 0, chassisTurnTimeUtil),
    std::make_unique<IterativePosPIDController>(
      chassisAnglekP, chassisAnglekI, chassisAnglekD, 0, chassisAngleTimeUtil),
    chassisGearset,
    adiScales);
  pidController->setVelocityMode(false);

  leftProfileController =
    std::make_shared<AsyncLinearMotionProfileController>(TimeUtilFactory().create(),
                                                         turnLimits,
                                                         std::make_shared<MotorGroup>(left),
  ...
  profileController = std::make_shared<AsyncMotionProfileController>(
    TimeUtilFactory().create(), turnLimits, skidSteerModel, chassisScales, chassisGearsetRatioPair);
  ...
  pursuitController =
    std::make_shared<PathFollower>(skidSteerModel, odom, chassisScales, lookahead, driveRadius);

  stopControllers();
}

void Chassis::linearProfileStraight(QLength idistance, QLength icurrentPos) {
  leftProfileController->generatePath(
    {icurrentPos.abs(), idistance.abs()}, "straight", straightLimits);
  rightProfileController->generatePath(
    {icurrentPos.abs(), idistance.abs()}, "straight", straightLimits);
  bool backward = false;
